...
export default class TimelineGenerator {
  ...
  generateTimelineEvents(heartbeats: Heartbeat[]): TimelineEvent[] {
    ...
    if (sortedHeartbeats.length > 0) {
      ...
      // console.log(`[TimelineGenerator] First heartbeat at ${new Date(firstHb.timestamp).toISOString()}, last at ${new Date(lastHb.timestamp).toISOString()}`);
    }

    // Group heartbeats into intervals
    const intervalGroups = this.groupHeartbeatsByInterval(sortedHeartbeats);
    // console.log(`[TimelineGenerator] Created ${Object.keys(intervalGroups).length} interval groups`);
  ...
  public groupHeartbeatsByInterval(heartbeats: Heartbeat[]): IntervalGroups {
    ...
    // console.log(`[TimelineGenerator] groupHeartbeatsByInterval: Processing ${heartbeats.length} heartbeats with interval ${this.aggregationInterval} min`);
    ...
    const intervalCounts = Object.entries(intervalGroups).map(([time, hbs]) => {
      return `${new Date(parseInt(time)).toISOString()}: ${hbs.length} heartbeats`;
    });
    
    if (intervalCounts.length > 0) {
      // console.log(`[TimelineGenerator] Interval distribution (showing first 5 of ${intervalCounts.length}):`);
      // intervalCounts.slice(0, 5).forEach(msg => console.log(`  - ${msg}`));
    } else {
      // console.log(`[TimelineGenerator] No valid intervals created!`);
    }
  ...
  generateActivities(intervalGroups: IntervalGroups): TimelineEvent[] {
    ...
    // Reduziere den Schwellenwert, um auch bei weniger Herzschlägen Events zu generieren
    const MIN_HEARTBEATS_RATIO = 0.1; // Von 0.5 auf 0.1 geändert, um mehr Events zu erzeugen
    // console.log(`[TimelineGenerator] generateActivities: Using MIN_HEARTBEATS_RATIO=${MIN_HEARTBEATS_RATIO}, expected ${this.aggregationInterval * 2 * MIN_HEARTBEATS_RATIO} heartbeats minimum per interval`);
    ...
    Object.entries(intervalGroups).forEach(([intervalStartStr, intervalHeartbeats]: [string, Heartbeat[]]) => {
      ...
      const minimumRequiredHeartbeats = Math.max(1, Math.ceil(expectedHeartbeats * MIN_HEARTBEATS_RATIO)); // Mindestens 1 Herzschlag erforderlich
      ...
      if (intervalHeartbeats.length >= minimumRequiredHeartbeats) {
        ...
        if (dominantActivity) {
            // console.log(`[TimelineGenerator] Interval ${new Date(intervalStartTime).toISOString()}: Dominant activity = ${dominantActivity.type}`);
            activities.push({
    ...
    // console.log(`[TimelineGenerator] generateActivities: Produced ${activities.length} activities from ${Object.keys(intervalGroups).length} intervals`);
    return activities;
  }
