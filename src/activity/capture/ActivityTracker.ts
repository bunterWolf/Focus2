...
export class ActivityTracker {
  ...
  private handleIntervalEnd(): void {
    ...
    if (this.isTracking && !this.useMockData) {
      // Forciere die Aggregation unabhängig vom Rückgabewert
      this.aggregationService.aggregateDay(this.currentDayKey);
      // Benachrichtige immer über Updates, unabhängig vom Ergebnis
      this.notifyDataUpdate(this.currentDayKey); 
      this.activityStorage.saveToDisk();
      console.log(`[Tracker] Interval handled: aggregation triggered and data update notified`);
    } else {
